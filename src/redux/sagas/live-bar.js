...
import { GO_TO_NEXT_SONG, GO_TO_PREVIOUS_SONG } from '../actions'
...
function* handleGoToNextSong() {
	...
	if (currentSet) {
		...
		if (nextSong) {
			//yield put(push(`/sets/${currentSet.id}/songs/${nextSong.id}`))
		} else {
...
function* handleGoToPreviousSong() {
	...
	if (prevSong) {
		//yield put(push(`/sets/${currentSet.id}/songs/${prevSong.id}`))
	} else {
